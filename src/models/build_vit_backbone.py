...
from .vit_adapter.vit import AdaptedVisionTransformer
...
from .vit_lora.vit import LoRAVisionTransformer
from .vit_ia3.vit import IA3VisionTransformer
from .vit_ssf.vit import SSFVisionTransformer
...
def build_vit_sup_models(
    cfg, model_type, crop_size, model_root=None, load_pretrain=True, vis=False
):
    ...
    if transfer_type == "prompt":
        ...
        model = PromptedVisionTransformer(model_type, crop_size, num_classes=-1, prompt_cfg=prompt_cfg, vis=vis)
    elif transfer_type == "adapter":
        adapter_cfg = cfg.MODEL.ADAPTER
        model = AdaptedVisionTransformer(model_type, crop_size, num_classes=-1, adapter_cfg=adapter_cfg, vis=vis)
    elif transfer_type == "lora":
        lora_cfg = cfg.MODEL.LORA
        model = LoRAVisionTransformer(model_type, crop_size, num_classes=-1, lora_cfg=lora_cfg, vis=vis)
    elif transfer_type == "ia3":
        ia3_cfg = cfg.MODEL.IA3
        model = IA3VisionTransformer(model_type, crop_size, num_classes=-1, ia3_cfg=ia3_cfg, vis=vis)
    elif transfer_type == "ssf":
        ssf_cfg = cfg.MODEL.SSF
        model = SSFVisionTransformer(model_type, crop_size, num_classes=-1, ssf_cfg=ssf_cfg, vis=vis)
    else:
